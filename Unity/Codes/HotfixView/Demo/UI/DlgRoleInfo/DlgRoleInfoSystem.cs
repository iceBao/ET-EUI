...
namespace ET
{
	...
	public static  class DlgRoleInfoSystem
	{
		...
		public static void RegisterUIEvent(this DlgRoleInfo self)
		{
			...
			self.View.E_AttributesLoopVerticalScrollRect.AddItemRefreshListener(((transform, index) =>
			{
			...
			RedDotHelper.AddRedDotNodeView(self.ZoneScene(), "AddAttribute", self.View.E_AttributePointText.gameObject, new Vector3(0.5f,0.5f,1), new Vector3(-17f, 10f, 0));
		}
		...
		public static void OnAttributeItemRefreshHandler(this DlgRoleInfo self, Transform transform, int index)
		{
			...
			scrollItemAttribute.E_attributeValueText.text = config.isPrecent == 0
					...
					$"{UnitHelper.GetMyUnitNumericComponent(self.ZoneScene().CurrentScene()).GetAsFloat(config.Id).ToString()}";
		}
