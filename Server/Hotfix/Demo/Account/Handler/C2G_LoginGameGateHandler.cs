        ...
        protected override async ETTask Run(Session session, C2G_LoginGameGate request, G2C_LoginGameGate response, Action reply)
        {
            ...
            using (session.AddComponent<SessionLockingComponent>())
            {
                using (await CoroutineLockComponent.Instance.Wait(CoroutineLockType.LoginGate, request.Account.GetHashCode()))
                {
                    ...
                    L2G_AddLoginRecod l2GAddLoginRecod = (L2G_AddLoginRecod) await MessageHelper.CallActor(loginCenterConfig.InstanceId, new G2L_AddLoginRecord()
                    {
                        AccountId = request.Account,
                        ServerId = scene.Zone
                    });

                    if (l2GAddLoginRecod.Error != ErrorCode.ERR_Success)
                    {
                        response.Error = l2GAddLoginRecod.Error;
                        reply();
                        session?.Disconnect().Coroutine();
                        return;
                    }
